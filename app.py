...
if analysis_type == "0. 正态性检验 (Normality Test)":
    ...
    st.markdown("**前提要求**：T检验、ANOVA 等工具均要求数据服从正态分布。**P > 0.05 代表正态**。")
...
# ================= 2. 双样本 T检验 =================
    ...
    with col1: d1_in = st.text_area("组别 A:", "15.2 14.8 15.5 14.9 15.1")
    with col2: d2_in = st.text_area("组别 B:", "16.1 15.9 16.5 16.2 16.0")
...
# ================= 3. 配对 T检验 =================
    ...
    st.markdown("比较**同一批产品**处理前后的变化（两组数据量必须完全相等）。")
    col1, col2 = st.columns(2)
...
# ================= 8. 双因素 ANOVA =================
    ...
    st.markdown("分析两个变量(因子)对结果的影响，并寻找 1+1>2 的**交互作用**。需输入三列数据，长度必须一致。")
    ...
    if st.button("运行分析"):
        ...
        if len(y) == len(fa) == len(fb) and len(y) > 0:
            ...
            try:
                # 使用 OLS 拟合模型，包含交互项 (FactorA:FactorB)
                ...
                st.error(f"模型运算出错，请检查数据格式是否正确（确保每个组合下都有数据支撑计算方差）。错误信息：{e}")
        else:
