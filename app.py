...
# ================= 7. 单因素 ANOVA =================
    ...
    st.markdown("""
    > 🎯 **应用场景**：比较【3个或以上独立组别】的均值是否有显著差异。例如：对比多家不同供应商交付零件的关键参数。
    ...
    for i in range(num_groups):
        group_name = chr(ord('A') + i)  # 自动生成 A, B, C, D...
        with cols[i % 3]:
            # 给前三组塞点默认数据，方便演示
            default_text = ""
    ...
    if st.button("运行分析"):
        ...
        if len(parsed_data) >= 2:
            # 这里的 *parsed_data 是 Python 神奇的“解包”操作，无论传入多少组都能算
